...
;
;
PRO hv_hmi_list2jp2_gs,list, drms=drms, wave_string=wave_string, do_reg=do_reg, $
...
;  storage.jp2_location = str_replace(storage.jp2_location, 'solarsan', 'brooklyn')
;
...
;
  for ii = 0,nl-1 do begin
     ...
     nz = n_elements(z)
     fitsname = z[nz-1]
     if exist(drms) then begin
       hd = drms[ii]
       read_sdo, fullname, hd, img, parent_out=concat_dir('$HOME','tmp'), /uncomp_delete, /mixed, $
	         /use_index ;, /use_shared  ; get image and data
     endif else begin
       read_sdo, fullname, hd, img, parent_out=concat_dir('$HOME','tmp'), /uncomp_delete, /mixed ;, /use_shared  ; get image and data
     endelse

     if keyword_set(do_reg) then begin
       which, 'aia_prep'
       aia_prep, hd, img, hd_reg, img_reg
       hd = hd_reg
       img = img_reg
     endif
...
;
        ...
        if not tag_exist(hd,'NAXIS1') then hd = add_tag(hd, n_elements(reform(img[*,0])),'NAXIS1')
...
loc = str_replace(loc,'/archive/sdo','/viz2')
        if not(is_dir(loc)) then spawn, ['mkdir', '-m', '775', loc], /noshell
;
...
if jcomm ne -1 then hd = rep_tag_value(hd, [hd.comment,''], 'COMMENT')
        jhist = where(tagnames eq 'HISTORY')
if jhist ne -1 then hd = rep_tag_value(hd, [hd.history,''], 'HISTORY')
...
spawn, ['chmod', '775', loc + jp2_filename], /noshell
spawn, ['ls', '-alF', loc + jp2_filename], /noshell
           print,progname + ' created ' + loc + jp2_filename

if not keyword_set(no_log_time) then begin
  t_create_ut = anytim(ut_time(!stime), /ccsds)
  t_create_ut_sec = anytim(t_create_ut)
  t_obs_ut = anytim(hd.date_obs, /ccsds)
...
;  t_obs_ut = tobs
  ...
  filnam_t_last = concat_dir(dir_logs, 'last_file_' + wave_string)     ; strtrim(hd.wavelnth,2))
