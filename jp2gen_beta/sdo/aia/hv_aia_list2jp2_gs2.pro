...
PRO hv_aia_list2jp2_gs2,list,$
                    ...
                    parent_out=parent_out, no_log_time=no_log_time
  progname = 'hv_aia_list2jp2_gs2'

  if not exist(parent_out) then parent_out = concat_dir(get_logenv('HOME'),'tmp')
;
; Line feed character:
;
  lf=string(10b)
...
;     img = readfits(fullname,hd)   ; get image and data
     if not tag_exist(hd,'wavelnth') then $
     read_sdo, fullname, hd, img, /use_index, parent_out=parent_out, /uncomp_delete, /mixed else $
     read_sdo, fullname, hd, img, parent_out=parent_out, /uncomp_delete, /mixed
...
;
     if hd.naxis1 eq 4096 then begin
        ...
        oJP2->SetData,img
        OBJ_DESTROY, oJP2
	spawn, ['chmod', '775', concat_dir(loc, jp2_filename)], /noshell
        print,' '
